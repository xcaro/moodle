...
Y.namespace('M.atto_indent').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    indentListItem: function () {
        var el = window.rangy.getSelection().anchorNode,
            root = this.getRootElement(),
            list = this.getParent(el, 'ul, ol', root);
        if (list) {
     ...
     * @return {(HTMLElement)[]|[]}
     */
    getParents: function (el, selector, root, collect) {
        var results = [];
        root = root || this.getRootElement() || document.body;
        var node = el;
        ...
        while (node) {
            ...
            if (selector.toUpperCase().indexOf(node.tagName) > -1) {
                if (collect) {
                    results.push(node);
                } else {
                    return [node];
    ...
    getParent: function (el, selector, root) {
        var parents = this.getParents(el, selector, root, false);
        return parents && parents.length > 0 ? parents[0] : null;
    },
    /**
     * Get root element is atto editor.
     *
